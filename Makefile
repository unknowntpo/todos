...
production/deploy/api:
	rsync -rP --delete ./bin/linux_amd64/api ./migrations todos@${production_host_ip}:~
	ssh -t todos@${production_host_ip} 'migrate -path ~/migrations -database $$TODOS_DB_DSN up'
...
production/configure/api.service:
	rsync -P ./remote/production/api.service todos@${production_host_ip}:~
