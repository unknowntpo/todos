...
# Prompt to enter a password for the PostgreSQL todos user (rather than hard-coding
# a password in this script).
read -p "Enter password for todos DB user: " DB_PASSWORD
...
# Set up the todos DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE todos"
sudo -i -u postgres psql -d todos -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d todos -c "CREATE ROLE todos WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the todos database to the system-wide environment 
# variables in the /etc/environment file.
echo "TODOS_DB_DSN='postgres://todos:${DB_PASSWORD}@localhost/todos'" >> /etc/environment
